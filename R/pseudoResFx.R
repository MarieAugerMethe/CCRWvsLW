...
pseudo.u.test <- function(U, nP, n, graph){

  # Binning the uniform pseudo residuals
  # Divinding the uniform distirbution in as many bins
  ...
  U_B <- hist(U,breaks=U_B,plot=F)$counts
  ...
  pValGT <- pchisq(GVal/wc,dfGT,lower.tail=F)
...
pseudo <- function(SL,TA_C,TA,SLmin,SLmax,missL,notMisLoc,n,mleMov,PRdetails,graph){
  ...
  # If graph=T get the best model
  # since only the pseudo residuals of the best model are shown
  if(graph){
    bestM <- grep("AIC",names(unlist(mleMov)))
  ...
  if(!any(is.na(mleMov$CCRW[1:6]))){
    ...
    Z[,1] <- pseudo.u.test(U_SL_CCRW, 3, n, graph[1])
    Z[,6] <- pseudo.u.test(U_TA_CCRW, 1, n, graph[1])
  }
  ...
  # nP = SLmin + mu
	Z[,2] <- pseudo.u.test(U_LW,2,n,graph[2])
  # nP = SLmin + mu + SLmax
	Z[,3] <- pseudo.u.test(U_TLW,3,n,graph[3])
  # nP = SLmin + lambda
	Z[,4] <- pseudo.u.test(U_E,2,n,graph[4]|graph[5])
  # nP = SLmin + lambda + SLmax
	Z[,5] <- pseudo.u.test(uTE,3,n,graph[6]|graph[7])

	######################
	# TA
  
  # nP = nothing!
	Z[,7] <- pseudo.u.test(U_U,0,n,graph[2]|graph[3]|graph[4]|graph[6])
  # nP = kapa
	Z[,8] <- pseudo.u.test(U_VM,1,n,graph[5]|graph[7])
